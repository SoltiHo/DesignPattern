...
namespace homework {
    ...
    namespace factory_method_problem {
        ...
        class DisplayPort {
        public:
            std::string send_video(int* res, Crypto crypto, int framerate) {
                std::string protocol;
                switch (crypto) {
                ...
                char str[80];
                sprintf_s(str, "  Send DisplayPort[%d,%d] via %s at %d fps.",
        ...
        public:
            std::string send_video(int* res, Crypto crypto, int framerate) {
                ...
                char str[80];
                sprintf_s(str, "  Send HDMI[%d,%d] via %s at %d fps.",
                    res[0], res[1], protocol.c_str(), framerate);
                return str;
            }
        };
        ...
        public:
            std::string send_video(int* res, Crypto crypto, int framerate) {
                ...
                char str[80];
                sprintf_s(str, "  Send MIPI[%d,%d] via %s at %d fps.",
                    res[0], res[1], protocol.c_str(), framerate);
                return str;
            }
        };
        ...
        public:
            std::string send_video(int* res, Crypto crypto, int framerate) {
                ...
                char str[80];
                sprintf_s(str, "  Send Widi[%d,%d] via %s at %d fps.",
                    res[0], res[1], protocol.c_str(), framerate);
                return str;
            }
        };
        ...
        void send_video(Display display, int* res, Crypto crypto, int framerate) {
            switch (display) {
            case DISPLAY_PORT:
                std::cout << DisplayPort().send_video(res, crypto, framerate) << "\n";
                break;
            case HDMI:
                std::cout << Hdmi().send_video(res, crypto, framerate) << "\n";
                break;
            case MIPI:
                std::cout << Mipi().send_video(res, crypto, framerate) << "\n";
                break;
            case WIDI:
                std::cout << Widi().send_video(res, crypto, framerate) << "\n";
                break;
